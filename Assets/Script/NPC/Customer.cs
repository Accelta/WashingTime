...
public class Customer : MonoBehaviour
{
    ...
    private enum State { MovingToLaundryQueue, WaitingForTurnToDrop, DroppingClothes, MovingToCleanQueue, WaitingForCleanClothes, TakingCleanClothes, Leaving }
    private State currentState = State.MovingToLaundryQueue;
    ...
    public void EnableCustomer()
    {
        isDisabled = false; // Assuming `isDisabled` is a flag that prevents the customer from functioning
        currentState = State.MovingToLaundryQueue; // Reset to moving to the laundry queue
    }
    private void Start()
    {
        laundryBasket = FindObjectOfType<LaundryBasket>();
    ...
    private void Update()
    {
        ...
        switch (currentState)
        {
            case State.MovingToLaundryQueue:
                MoveToLaundryQueue();
                break;
            case State.WaitingForTurnToDrop:
                WaitForTurnToDrop();
                break;
            case State.DroppingClothes:
                DropDirtyClothes();
    ...
    private void MoveToLaundryQueue()
    {
        transform.position = Vector3.MoveTowards(transform.position, laundryQueuePosition, moveSpeed * Time.deltaTime);
        if (Vector3.Distance(transform.position, laundryQueuePosition) < 0.1f)
        {
            currentState = State.WaitingForTurnToDrop;
        }
    }

    private void WaitForTurnToDrop()
    {
        if (laundryQueue.Peek() == this)
        {
            currentState = State.DroppingClothes;
        }
    }
